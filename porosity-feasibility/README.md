...
Customer-facing summary: the accompanying 10-slide presentation (provided separately).
...
the accompanying 10-slide presentation for the customer-facing framing.
...
`figures/eda_measurements_by_sample.html`, `figures/actual_vs_predicted.html`, and
`figures/thermal_correlation.html` (standalone interactive versions of the same figures
embedded in the notebook) in place.
...
```
porosity-feasibility/
├── README.md                              this file
├── notebook_revision_notes.md              changelog from the final correction pass
├── requirements.txt                        pinned-loose Python dependencies
├── porosity_pipeline.ipynb                 primary/submitted notebook
├── porosity_explainability_attempt.ipynb   companion notebook
├── porosity_feasibility.ipynb              earlier, more exhaustive exploratory notebook
├── data/                                   raw input files (see Data, above)
└── figures/                                standalone interactive HTML figures
```
...
- `porosity_pipeline.ipynb` — **primary/submitted notebook.** Customer clarifications and
  ...
  Environment/setup); interactive `.html` versions in `figures/` are always written
  regardless.
...
**`figures/`** — every standalone interactive figure, generated by the notebooks above
(open directly in a browser for hover/zoom/legend-isolate — GitHub's notebook viewer
doesn't execute the JS these need, which is why each is also embedded as a static PNG
directly in its source notebook):
- `eda_measurements_by_sample.html`, `actual_vs_predicted.html`, `thermal_correlation.html`
  — from `porosity_pipeline.ipynb`.
- `pixel_importance.html`, `pixel_importance_grid.html` — from
  `porosity_explainability_attempt.ipynb`.
- `pixel_importance_dashboard.html` — a standalone, presentation-oriented dashboard built
  from the same pixel-importance data as the two figures above: headline stats, a full
  32×32 heatmap of every pixel's importance with the top-20 outlined, a ranked bar chart
  with LOO-stability labels, and all 20 samples' actual thermal images (shared color
  scale, sorted by porosity) with the important region outlined on each. Not generated by
  either notebook's code — built separately for presenting these results to a
  non-technical audience; same underlying numbers and caveats as the notebook sections
  it's drawn from (Sections 7-10 of `porosity_explainability_attempt.ipynb`).
- `thickness_effect.html` — supplementary figure: effect of adding thickness as a
  predictor, not generated by either notebook above or part of the primary pipeline's
  stages.

**Other files:**
- `notebook_revision_notes.md` — changelog documenting a final consistency/disclosure
  correction pass applied to both notebooks.
